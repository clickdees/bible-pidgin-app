...
async function getPCMVerse(ref) {
  ...
  // Loop through every verse in the file
  $('span.verse, .verse').each((_, el) => {
    const verseNum = parseInt($(el).text().trim());
    
    // If this verse is inside our requested range
    if (verseNum >= startVerse && verseNum <= endVerse) {
      // Get the entire parent container that holds this verse and its lines
      const container = $(el).parent();

      // Collect ALL text inside this container (q, q2, p, etc.)
      let verseText = container.text();
  ...
  // Very strong fallback: regex on the whole body
  if (!fullText) {
...
async function createGraphic(data, isPidgin) {
  ...
  ctx.fillText(isPidgin ? "PCM Pidgin Bible" : "English (NLT)", centerX, img.height - 80);
