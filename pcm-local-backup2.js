...
function parseRef(ref) {
  ...
  // Clean the reference (remove extra spaces, "of", etc.)
  ...
  // Handle ranges like "Proverbs 3:5-6" → we only need the chapter
  // We will always take the FIRST verse in the range
  const rangeMatch = clean.match(/^(\d?[a-z]+)(\d+):(\d+)(?:-(\d+))?$/);
  if (!rangeMatch) return null;

  let bookName = rangeMatch[1];
  const chapter = parseInt(rangeMatch[2]);
  const verse = parseInt(rangeMatch[3]);   // first verse in range

  const bookCode = bookMap[bookName];
  if (!bookCode) {
    console.log(`   ⚠️ Unknown book: ${bookName}`);
    return null;
  }

  // Psalms use 3-digit chapter (PSA001.htm), others use 2-digit
  const padLength = (bookCode === 'PSA') ? 3 : 2;
  const chapterStr = chapter.toString().padStart(padLength, '0');

  return { bookCode, chapterStr, verse };
}

// ==================== IMPROVED getPCMVerse ====================
async function getPCMVerse(ref) {
  ...
  // Stronger search for verse number
  $('sup, .verse, span, b, strong, p').each((_, el) => {
    const num = $(el).text().trim();
    if (num === verse.toString() || num === `${verse}.` || num === `${verse}`) {
      // Get the text after the verse number
      let next = $(el).nextAll().first().text() || $(el).parent().text();
      text = next.replace(/^\s*\d+\s*[:.]?\s*/, '').trim();
      return false; // stop loop
    }
  });

  // Final fallback: regex on whole body
  if (!text) {
    const bodyText = $('body').text().replace(/\s+/g, ' ');
    const regex = new RegExp(`\\b${verse}\\s*[:.]?\\s*([^\\d].*?)(?=\\s+\\d+\\s*[:.]?|$)`, 'i');
    const match = bodyText.match(regex);
    if (match) text = match[1].trim();
  }

  return text || `Verse ${verse} not found in ${filename}`;
}
